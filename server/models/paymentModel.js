...
module.exports = {
  // select bill.name, bill.dueDate, payment.amount, users.name AS whoIsOwed, payment.id AS paymentId, bill.id AS billId from payment left outer join bill on (payment.billid=bill.id) left outer join users on (users.id=bill.userid) where payment.paid=0;
  // money the user owes
...
// select users.name AS ower, payment.amount, bill.name, bill.dueDate, bill.id AS billId, payment.id AS paymentID from bill left outer join payment on (bill.id=payment.billid) left outer join users on(payment.userid=users.id) where bill.userId = ? AND payment.paid=0
